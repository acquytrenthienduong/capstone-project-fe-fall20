<template>
  <v-row no-gutters justify="center">
    <v-dialog v-model="dialog" persistent max-width="700">
      ...
      <v-card>
        <v-card-title class="title px-8 py-4"> THÔNG TIN KHÁCH HÀNG </v-card-title>
        ...
        <v-card-actions class="py-4 px-8">
          ...
          <v-btn color="red darken-1" text @click="dialog = false"> THOÁT </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-dialog v-model="changePasswordDialog" persistent max-width="390">
        <v-card>
          ...
          <v-container class="px-10 pt-10">
            ...
            <h3 v-if="checkCurrentPassword">Mật khẩu hiện tại của bạn không đúng!</h3>
          </v-container>
          <v-card-actions class="px-10 pb-10">
            <v-spacer></v-spacer>
            <v-btn color="red darken-1" text @click="reset"> Hủy </v-btn>
            <v-btn color="green darken-1" text @click="changePassdRule"> Lưu </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
    ...
    <v-row justify="center">
      <v-dialog v-model="changePhoneDialog" persistent max-width="390">
        <v-card>
          ...
          <v-container class="px-10 pt-10">
            <v-text-field label="Nhập số điện thoại mới" outlined></v-text-field>
            <v-text-field outlined label="Nhập mật khẩu"></v-text-field>
          </v-container>
          <v-card-actions class="px-10 pb-10">
            <v-spacer></v-spacer>
            <v-btn color="black darken-1" class="white--text" @click="moveToStep2">
              Tiếp tục
            </v-btn>
      ...
      <v-dialog v-model="confirmPhoneDialog" persistent max-width="390">
        <v-card>
          ...
          <v-card-actions class="px-10 pb-10">
            ...
            <v-btn color="red darken-1" text @click="confirmPhoneDialog = false">
              Hủy
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    changePassdRule() {
      if (this.oldPassword != "" && this.newPassWord != "" && this.confimPassword != "") {
        this.checkEmpty = false;
        if (this.newPassWord === this.confimPassword) {
          this.checkMatchPW = false;
          axios
            .post(this.host + "/changePassword/" + this.customerId, {
